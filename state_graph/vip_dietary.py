    ...
    async def lats_search(state: VipDietaryState) -> dict:
        ...
        for candidate in untried:
            prompt = (
                ...
                "Assume our executive chef will prepare these beautifully. Does this make a coherent two-course banquet menu? "
                "Reply with exactly YES on the first line."
            )
...
def make_chef_signoff(db_path: str):
    async def chef_signoff(state: VipDietaryState) -> dict:
        open_hitl_ticket(
            graph_id="vip_dietary_agent",
            thread_id=state.get("_thread_id", "unknown"),
            reason=(
                f"Mandatory executive chef sign-off for guest {state.get('guest_id')} "
                f"proposed pairing {state.get('current_combo')}."
            ),
...
def build_vip_dietary_graph(llm_generate=None, check_stock=None,
    ...
    return builder.compile(checkpointer=checkpointer, interrupt_before=["record_chef_decision"])